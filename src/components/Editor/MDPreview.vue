...
import { defineProps, defineEmits, toRefs, ref, watch, onMounted, onBeforeUnmount, computed } from 'vue';
...
<template>
  <div v-bind='$attrs' class='w-full flex flex-row'>
        ...
        <div class='flex mt-3 text-gray-400 items-center'>
          ...
          <div class='flex flex-col'>
            ...
            <div>
              ...
              <a v-if='isEdit' @click='() => edit(data?.id)'
                class='ml-6 cursor-pointer text-indigo-500 hover:underline'>编辑</a>
            </div>
          </div>
        </div>
        <div v-if='data?.column' class="column-info mt-6 p-3 border border-gray-100 rounded flex gap-2 items-center">
          <div @click='() => toColumnDetail(data?.column?.id)'
            class='flex gap-2 items-center cursor-pointer text-gray-600 hover:text-indigo-500'>
            <img :src='data?.column?.cover?.url' class='w-9 h-9'
              @error='(e) => (e.target as HTMLImageElement).src = "/img/column-default-cover.webp"'>
            <span class="name">{{ data?.column?.name }}</span>
          </div>
          <span class="detail text-gray-400 text-sm flex-1">专栏收录该文章</span>
          <span class="text-gray-400 text-sm flex gap-4">
            <span>{{ data?.column?.subscribeCnt }} 订阅</span>
            <span>{{ data?.column?.articleCnt }} 篇文章</span>
          </span>
        </div>
      </div>
    ...
    <div class='rightSider relative w-1/4' style='width: 260px; padding-left: 20px;'>
      <div id='myPreviewEditorSider' class='fixed bg-white'  :class='[isShowHeader ? "top-28" : "top-8"]' style='transition: 300ms;'>
        <nav style='height: 540px' class='relative'>
          ...
          <div id='myPreviewEditorOutlineList' class='overflow-y-auto overflow-x-hidden absolute right-0 w-full'
            ...
            <List :data-list='outlineNodeList' @click='outlineLinkContent' class='w-full'
