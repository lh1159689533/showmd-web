 ...
 */
function querySelector(node: Element | Document = document, selector: string): Element {
 ...
 * @param selector css选择器
 */
function querySelectorAll(node: Element | Document = document, selector: string): Element[] {
  return toArray<Element>(node.querySelectorAll(selector));
}
