        ...
        <div class="article-list-item flex justify-between text-sm text-gray-800 px-6 py-4 border-t hover:bg-gray-50">
          <div class="article-list-item-content flex flex-col flex-1">
            ...
            <div class="flex mt-4 justify-between">
              ...
              <List :data-list="oprateList" class="oprate flex gap-4 text-gray-600" item-class="hover:text-indigo-500 cursor-pointer" @click="(item) => item.handle?.(article?.id)">
                <template #default="{ item }">
                  <span :class="[item.key === 'del' ? 'hover:text-red-500' : 'hover:text-indigo-500']">{{ item.title }}</span>
                </template>
              </List>
            </div>
          </div>
