...
<template>
  <el-drawer :model-value='visible' :title='column?.name' direction='rtl' :close-on-click-modal='false'
    :before-close='onBeforeClose' destroy-on-close :size='600'>
    <div class='flex items-center justify-between pl-3 mb-2'>
      <el-checkbox @change='toggleAllSelection'
    ...
    <Draggable v-if='column?.articles?.length' v-model="column.articles" item-key="id" :component-data='{
      type: "transition-group",
      name: !drag ? "flip-list" : null
    }' @start='drag = true' @end='onSortEnd' v-bind='dragOptions'>
      <template #item='{ element: article }'>
        <div
          class='article-list-item flex gap-8 items-center cursor-move text-sm text-gray-800 px-3 py-2 border-b hover:bg-gray-50'>
  ...
  <MoveArticle v-if='showMoveDialog' :columnId='id' :article-ids='selectionArticles.map(item => item.id)'
