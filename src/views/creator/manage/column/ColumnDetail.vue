...
watchEffect(() => {
  if (route.params?.id) {
    ...
    findById(columnId).then(data => {
      column.value = data;
    }).catch();
    findArticleList(columnId).then(data => {
...
<template>
  <div class='my-column-list my-8 bg-white'>
    ...
    <List v-else-if='articleList?.length' :data-list='articleList'>
      <template #default='{ item: article }'>
        <div
          ...
          class='article-list-item flex justify-between cursor-pointer text-sm text-gray-800 px-6 py-4 border-t hover:bg-gray-50'
        >
    ...
    <Empty v-else class='border-t'>
      <div v-if='user?.id === column?.userId'>
        还没有文章，<span @click='toManageColumn' class='ml-2 text-indigo-500 cursor-pointer hover:underline'>快去收录文章吧</span>
      </div>
