...
<template>
  <div class="my-column-list mt-20 mb-8 bg-white dark:bg-zinc-900">
    <div class="header flex flex-col justify-between px-6 py-4">
      <div class="flex gap-8 mb-4">
        ...
        <div class="detail flex-1 flex flex-col gap-2">
          <div class="title flex items-center gap-2">
            <span
              class="attrs px-1 text-sm text-indigo-500 bg-indigo-100 dark:bg-zinc-800"
              :style="{ borderRadius: '1px' }"
            >{{
              column?.isPrivate ? '私有' : '公开' }}</span>
            <span class="text-xl flex-1 dark:text-zinc-300">{{ column?.name }}</span>
          </div>
          <span class="desc text-gray-600 text-sm my-2 dark:text-zinc-500">{{ column?.desc }}</span>
          <div class="flex flex-1 gap-4 text-sm text-gray-500 dark:text-zinc-500">
      ...
      <div class="user p-3 rounded bg-gray-50 dark:bg-zinc-800">
        <span class="text-base font-bold text-gray-500 dark:text-zinc-300">
          作者: <span class="pl-4">{{ column?.user?.name }}</span>
        </span>
        <div class="flex gap-4 text-sm mt-3 text-gray-500 dark:text-zinc-500">
    ...
    <List v-else-if="articleList?.length" :data-list="articleList">
      <template #default="{ item: article }">
        <div
          ...
          class="article-list-item flex justify-between cursor-pointer text-sm px-6 py-4 border-t text-gray-800 hover:bg-gray-50 dark:border-zinc-800 dark:hover:bg-zinc-800"
        >
          <div class="article-list-item-content flex flex-col flex-1">
            <div class="title text-xl mb-3 text-gray-900 hover:text-indigo-600 dark:text-zinc-300 dark:hover:text-indigo-600">
              {{ article.name }}
            </div>
